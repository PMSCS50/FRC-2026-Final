...
public class ProfiledSSController<Inputs extends Num, Outputs extends Num> {
    ...
    public double calculate(Matrix<Outputs, N1> currentOutputs) {
        ...
        // FIX 2: Convert setpoint matrix to TrapezoidProfile.State for the goal argument
        TrapezoidProfile.State goal = new TrapezoidProfile.State(setpoint.get(0, 0), setpoint.get(1, 0));
        trapezoidState = profile.calculate(dtSeconds, trapezoidState, goal);

        // FIX 3: setNextR() takes a Matrix<N2,N1>, not two separate doubles
        stateSpaceLoop.setNextR(VecBuilder.fill(trapezoidState.position, trapezoidState.velocity));
        ...
        // FIX 5: Was calling nonexistent isAtSetpoint(error); correct method is isAtSetpointInternal(error)
        atSetpoint = isAtSetpointInternal(error);
    ...
    private Matrix<Outputs, N1> fuseSensorReadings(Matrix<Outputs, N1> primarySensorOutput) {
        // FIX 6: Fuse all output rows, not just index 0, to match actual Outputs dimensionality
        int numRows = primarySensorOutput.getNumRows();
        double[] fusedValues = new double[numRows];

        for (int i = 0; i < numRows; i++) {
            double primary = primarySensorOutput.get(i, 0);
            // Use lastSecondaryMeasurement only for row 0 (position); pass through primary for others
            double secondary = (i == 0) ? lastSecondaryMeasurement : primary;
            fusedValues[i] = primary * (1.0 - secondarySensorWeight) + secondary * secondarySensorWeight;
        }

        lastPrimaryMeasurement = primarySensorOutput.get(0, 0);

        // Build the fused output matrix from the computed values
        Matrix<Outputs, N1> result = new Matrix<>(outputsNat, Nat.N1());
        for (int i = 0; i < numRows; i++) {
            result.set(i, 0, fusedValues[i]);
        }
        return result;
    }
    ...
    public void reset(Matrix<N2, N1> currentState) {
        ...
        // FIX 1 (continued): This reassignment is now valid since trapezoidState is no longer final
    ...
    /**
     * Returns true if the trapezoidal profile has reached the goal AND the
     * state estimate is within tolerance.
     *
     * FIX 4: Replaced profile.isFinished(dtSeconds) — that API checks whether a
     * profile of *total duration* dtSeconds is done, not whether the running
     * profile has converged. Instead, compare trapezoidState directly to the goal.
     */
    public boolean isAtSetpoint() {
        TrapezoidProfile.State goal = new TrapezoidProfile.State(setpoint.get(0, 0), setpoint.get(1, 0));

        boolean profileDone =
            Math.abs(trapezoidState.position - goal.position) < tolerance.get(0, 0) &&
            Math.abs(trapezoidState.velocity - goal.velocity) < tolerance.get(1, 0);
